...
function edusharing_get_auth_key() {
    ...
    switch($eduauthkey) {
        ...
        case 'randomUId':
            $usr_ident = getUserIdent() . '@' . ilCmiXapiUser::getIliasUuid() . '.ilias';
...
function getUserIdent() : string
{
    ...
    $usrIdent = getUserObjectUniqueId(32);
 ...
 * @return string
 */
function getUserObjectUniqueId( $length = 32 ) : string
{
    $id = ilCmiXapiUser::getUUID($length);
    $exists = userUniqueIdExists($id);
    while( $exists ) {
        $id = ilCmiXapiUser::getUUID($length);
        $exists = userUniqueIdExists($id);
    }
...
function userUniqueIdExists($id): bool
{
...
function edusharing_get_auth_data() {
    ...
    $eduauthparamnameuseridValue = edusharing_get_auth_key();
	$eduauthparamnameuserid = $settings->get('EDU_AUTH_PARAM_NAME_USERID');
	if (empty($eduauthparamnameuserid)) {
		$eduauthparamnameuserid = '';
        $eduauthparamnameuseridValue = '';
	}

    $eduauthparamnamelastnameValue = $DIC->user()->lastname;
	...
	if (empty($eduauthparamnamelastname)) {
        ...
        $eduauthparamnamelastnameValue = '';
	}

    $eduauthparamnamefirstnameValue = $DIC->user()->firstname;
    ...
    // Set by external sso script. Do not change to moodle $SESSION!
		...
		$authparams = array(
			array('key'  => $eduauthparamnameuserid, 'value'  => $eduauthparamnameuseridValue),
			array('key'  => $eduauthparamnamelastname, 'value'  => $eduauthparamnamelastnameValue),
			array('key'  => $eduauthparamnamefirstname, 'value'  => $eduauthparamnamefirstnameValue),
			array('key'  => $eduauthparamnameemail, 'value'  => $eduauthparamnameemailValue),
			array('key'  => 'affiliation', 'value'  => $eduauthaffiliation),
			array('key'  => 'affiliationname', 'value' => $eduauthaffiliationname)
		);
    }
